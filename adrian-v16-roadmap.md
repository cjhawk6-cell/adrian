...
- **The `ai_sessions` table** — still not created in the live Supabase project as of this check (re-verified live: still a 404). The fix has been sitting in `supabase-schema.sql` since Phase 9 (safe to re-run in full); this needs someone with a Supabase credential I don't have access to run it in the SQL Editor. Not something I can close out myself.
